...
if not initialized then
    ...
    local emote_list = {
        ...
        --以下、エモート追加処理
        all_spells = {id="all_spells"} ,
        all_rockets = {id="all_rockets"} ,
        all_deathcrosses = {id="all_deathcrosses"} ,
        all_discs = {id="all_discs"} ,
        all_nukes = {id="all_nukes"} ,
        all_acid = {id="all_acid"} ,
        all_blackholes = {id="all_blackholes"} ,
        bloodtentacle = {id="bloodtentacle"} ,
        broken_spell = {id="broken_spell"} ,
        chaos_polymorph_field = {id="chaos_polymorph_field"} ,
        empty = {id="empty"} ,
        enlarge = {id="enlarge"} ,
        generate_random_deck_5 = {id="generate_random_deck_5"} ,
        gun = {id="gun"} ,
...
--[[
    ...
    local misobon_emote_list_ordered = {
        "bomb" ,
        "heart",
        "longleg",
        "barfer",
    ...
    local function draw_player_info(player, userId)
        if (player.sampo) then
            ...
            --以下、サンポの画像の位置をズラす処理
            GuiImage(gui, next_id(), 95, 0, "mods/noita-together/files/ui/sampo.png", 0.5, 1, 1)
            --以上、サンポの画像の位置をズラす処理
        end
        ...
        --以下、死亡したプレイヤーが全プレイヤーのエモートを見れるようにする処理
        ...
        if (player.emote ~= nil and emote_list[player.emote] ~= nil and not player.emoteIsNemesisAblility and ((player.team ~= nil and NEMESIS.nt_nemesis_team ~= nil and player.team == NEMESIS.nt_nemesis_team) or (NEMESIS.alive == false))) then
        --以上、死亡したプレイヤーが全プレイヤーのエモートを見れるようにする処理
            --FIXME drawEmote
        ...
        if (show_send_gold) then
            if(NEMESIS~=nil and NEMESIS.nt_nemesis_team~=nil and player.team~=nil and NEMESIS.nt_nemesis_team==player.team and player.curHp > 0) then
                ...
                if (GuiImageButton(gui, next_id(), 100, 0, "", "data/ui_gfx/items/goldnugget.png")) then
                    ...
                    --以下、teams送金システムの範囲を広げる処理
                    dofile("mods/Nemesis-Ability-Plus/teams_append/sendgold.lua")
                    --以上、teams送金システムの範囲を広げる処理
        ...
        --以下、死亡したプレイヤーは全てのプレイヤーの装備を見れる処理
        --if (player.team ~= nil and NEMESIS.nt_nemesis_team ~= nil and player.team == NEMESIS.nt_nemesis_team) then
        if ((player.team ~= nil and NEMESIS.nt_nemesis_team ~= nil and player.team == NEMESIS.nt_nemesis_team)or(NEMESIS.alive == false)) then
        --以上、死亡したプレイヤーは全てのプレイヤーの装備を見れる処理
    ...
    local function draw_winner(base_x, base_y, team)
        ...
        elseif(aCount <= 1) then
            ...
            GuiZSetForNextWidget(gui, 8196)
			GuiImage(gui, next_id(), base_x-50, base_y-10, "data/items_gfx/emerald_tablet.png", 1, 2, 2, 0)
			GuiText(gui, base_x-20, base_y, "Interim Report")
			--以上、改変部分3
        end
    end
    ...
    local function draw_emote_select() 
        ...
        for _, emoteid in pairs(emote_list_ordered) do
            ...
            if (GuiImageButton(gui, next_id(), offset_x, 0, "", emote_list[emoteid].sprite or "data/ui_gfx/gun_actions/"..emoteid..".png")) then
                ...
                dofile("mods/Nemesis-Ability-Plus/teams_append/sendemote.lua")
                sendEmote( emote_list[emoteid], target )
            end
            offset_x = (offset_x + 20) % 220
        end
        GuiLayoutEnd(gui)
        GuiText( gui, 2, 0, "Emote" )
        GuiEndScrollContainer(gui)
    end

    local function draw_misobon_emote_select() 
        GuiZSetForNextWidget(gui, 10)
        GuiBeginScrollContainer(gui, next_id(), 450, 50, 100, 60, false, 1, 1)
        GuiLayoutBeginVertical(gui, 0, 0)
        if (GuiImageButton(gui, next_id(), 88, 0, "", "mods/noita-together/files/ui/close.png")) then
            show_emote_select = false
        end
        local offset_x = 0
        for _, emoteid in pairs(misobon_emote_list_ordered) do
            if (offset_x < 60) then
                GuiOptionsAddForNextWidget(gui, GUI_OPTION.Layout_NextSameLine)
            end
            if (GuiImageButton(gui, next_id(), offset_x, 0, "", emote_list[emoteid].sprite or "data/ui_gfx/gun_actions/"..emoteid..".png")) then
            ...
            offset_x = (offset_x + 20) % 80
        end
        GuiLayoutEnd(gui)
        GuiText( gui, 2, 0, "Misobon" )
        GuiEndScrollContainer(gui)
    end
    ...
    function draw_gui()
		...
		if (show_player_list or GlobalsGetValue("AUTO_SHOW_PLAYER_LIST")=="1") then
		--以上、プレイヤーリストを開始時に自動で開く処理
            if (not last_inven_is_open) then
                draw_player_list(PlayerList)
				--以下、プレイヤーリストを開始時に自動で開く処理
				if (GlobalsGetValue("AUTO_SHOW_PLAYER_LIST")=="1") then
					show_player_list = true
					GlobalsSetValue("AUTO_SHOW_PLAYER_LIST","0")
				end
				--以上、プレイヤーリストを開始時に自動で開く処理
            end
        end
        ...
        --以下、死亡時にアイコンを出す処理。Thanks for Syagami!
        if(NEMESIS ~= nil) then
            if(NEMESIS.deaths > 0) then
                GuiImage(gui, next_id(), screen_width - 20, 5, "data/ui_gfx/gun_actions/necromancy.png", 1, 0.5, 0.5)
            end
        end
        --異常、死亡時にアイコンを出す処理。
    end
