...
function sendEmote( emote, target )    
    ...
    --[[
    if (type(emote.delay) == "number") then
        GamePrintImportant(emote.delay)
    end
    ]]
    local queue = json.decode(NT.wsQueue)
    table.insert(queue, {event="CustomModEvent", payload={name="NemesisTeamSendEmote", team=team, emote=emote.id, misobon=emote.misobon, entity=emote.entity, target=target, displayName=displayName, delay=emote.delay}})
